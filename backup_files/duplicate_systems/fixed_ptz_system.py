...
class FixedPTZController:
    ...
    def send_command(self, action):
        ...
        # ✅ 修复的命令映射 - 使用正确的panLeft正负值
        command_map = {
            'up': {"method": "ptz_move_start", "param": {"channelid": 0, "tiltUp": 120}},
            'down': {"method": "ptz_move_start", "param": {"channelid": 0, "tiltUp": -120}},
            'left': {"method": "ptz_move_start", "param": {"channelid": 0, "panLeft": 120}},     # ✅ 正确
            'right': {"method": "ptz_move_start", "param": {"channelid": 0, "panLeft": -120}},   # ✅ 修复：使用panLeft负值
            ...
            if success:
                try:
                    ...
                    if 'error' in response:
                        ...
                        if error_code == 0:
                            print(f"✅ PTZ命令 {action}: 成功 - {data['param']}")
                            return True
                        else:
...
class WebCameraVLM:
    ...
    def generate_fake_frame(self):
        ...
        cv2.putText(frame, f"Fixed PTZ Parameters", (20, 50),
                   cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(frame, f"Time: {timestamp}", (20, 100),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(frame, f"Left/Right: panLeft +/-120", (20, 150),
...
<body>
    <h1 class="title">🎥 修复版PTZ监控 - 正确的左右移动参数</h1>
    ...
    <div class="container">
        ...
        <div class="controls">
            ...
            <div class="status" id="status">
                ...
                <p>参数: <span id="param-status">已修复</span></p>
            </div>
            ...
            fetch(`/api/ptz/${action}`, { method: 'POST' })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.fixed_params) {
                            document.getElementById('param-status').textContent = '✅ 修复参数';
                        }
                    } else {
...
'''
    ...
    parser.add_argument('--port', type=int, default=5004, help='Web服务器端口')
    parser.add_argument('--host', default='0.0.0.0', help='Web服务器主机')

    args = parser.parse_args()

    print("🎯 修复版PTZ监控系统 - 正确的左右移动参数")
    print("=" * 60)
    print("🔧 修复内容: 使用 panLeft 正负值控制左右移动")
    print("   左移: panLeft: 120")
    print("   右移: panLeft: -120")
